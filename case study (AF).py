...
def AF_model(df, model_name ='dSurv_af.pkl', model_path = 'model_result_af_perprotocal', load = False, build_data = True, layers = 30):
    ...
    if load:
        ...
        modelDsurv, history_dict = create_model(rnn_x.shape[2], max_time, history_itvl, data, val_data, lstm_window= 6,
                                                alpha=1.5,
                                                beta= 1, gamma=0.5, load=load, verbose=0, model_name=model_name,
    ...
    def gradient_importance(data,pred_time, model, tp = 0):
    ...
    # grads = gradient_importance(pred_data, pred_time, model = modelDsurv, tp = 0)
    # var_name = Colnames.copy()
    # var_name[0] = 'NOAC'
