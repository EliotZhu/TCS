...
data, val_data, test_data, train_stat, val_stat, test_stat, surv_func_wrapper, train_full, val_full, test_full, raw = \
    get_data(input_dim=6, sampleSize=1000, max_time=30, prediction_itvl=1, history_itvl=14,
             overlap=0.5, seed=1234, std=0.1, confound=0.5, scale=5)
...
modelCDSM,model_p, history_dict = create_model(rnn_x.shape[2], max_time, history_itvl, data, val_data, lstm_window= 7,
                                        alpha= 2, beta= 1, gamma= 0, load=False, verbose=0, model_name='CDSM',
...
cfl = cf_cdsm - 1.96 * cf_std_1_adj
cfu = cf_cdsm + 1.96 * cf_std_1_adj
#############################################
...
ax.plot(range(max_time), np.mean(cf_durv, 0), '+', color='#8DBFC5', alpha=0.9, label="CDSM (na)")
ax.plot(range(max_time), np.mean(cf_cdsm, 0), color='#8DBFC5', alpha=1, label="CDSM")
ax.fill_between(range(max_time), np.mean(cfl,0), np.mean(cfu,0), color='#8DBFC5', alpha=0.2)
